...
import { useApp, type MemoryImportance } from '../data/store';
import { LuCalendar, LuPlus, LuX, LuBrain, LuTrash2, LuSave, LuFlame } from 'react-icons/lu';
...
export function RememberView() {
  ...
  const [importance, setImportance] = useState<MemoryImportance>(1);

  const handleAdd = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newTitle.trim()) return;
    addMemory(newTitle, newDetail, importance);
    setNewTitle('');
    setNewDetail('');
    setImportance(1);
    setIsAdding(false);
  ...
  const getFontSize = (level: MemoryImportance) => {
    if (level === 3) return "text-2xl font-black text-sky-400";
    if (level === 2) return "text-lg font-bold text-zinc-200";
  ...
  return (
    <div className="flex-1 flex flex-col bg-black/40 border border-zinc-900 rounded-3xl overflow-hidden shadow-2xl">
      <div className="p-5 border-b border-zinc-950 flex justify-between items-center bg-zinc-950/50">
        <div className="flex items-center gap-2">
          <LuBrain size={14} className="text-sky-500" />
          <SectionLabel className="mb-0 text-sky-500 tracking-wide text-xs">Remember Things</SectionLabel>
        </div>
        <button
          onClick={() => setIsAdding(!isAdding)}
          className={`transition-all transform hover:scale-125 active:scale-95 ${isAdding ? 'text-red-500 rotate-90' : 'text-sky-600 hover:text-sky-400'}`}
        >
          {isAdding ? <LuX size={18} /> : <LuPlus size={20} />}
        </button>
      </div>

      <div className="p-5 flex-1 overflow-y-auto custom-scroll flex flex-col gap-6">
        {isAdding && (
          <form onSubmit={handleAdd} className="flex flex-col gap-3 p-4 bg-zinc-950 rounded-2xl border border-dashed border-sky-500/30 animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center mb-1">
              <span className="text-[10px] font-black text-zinc-600 uppercase tracking-widest">Weighting</span>
              <div className="flex gap-2">
                {[1, 2, 3].map((v) => (
                  <button
                    key={v}
                    type="button"
                    onClick={() => setImportance(v as MemoryImportance)}
                    className={`w-7 h-7 rounded-lg text-[10px] font-black transition-all ${importance === v ? 'bg-sky-500 text-white shadow-glow-blue scale-110' : 'bg-zinc-900 text-zinc-600 hover:text-zinc-400'}`}
                  >
                    {v === 1 ? 'S' : v === 2 ? 'M' : 'L'}
                  </button>
                ))}
            ...
            <input
              className="bg-black border border-zinc-900 rounded-lg text-sm p-3 text-zinc-100 focus:ring-1 focus:ring-sky-500 outline-none font-bold"
              placeholder="Core Thought..."
              value={newTitle}
              onChange={e => setNewTitle(e.target.value)}
              autoFocus
            />
            <textarea
              className="bg-black border border-zinc-900 rounded-lg text-[11px] p-3 text-zinc-400 focus:ring-1 focus:ring-sky-500 outline-none h-20 resize-none"
              placeholder="Supporting Details..."
              value={newDetail}
              onChange={e => setNewDetail(e.target.value)}
            />
            <button type="submit" className="text-[10px] bg-sky-500/10 text-sky-400 font-black uppercase py-2.5 rounded-lg border border-sky-500/20 hover:bg-sky-500 hover:text-white transition-all flex items-center justify-center gap-2">
              <LuSave size={12} />
              Persist to Memory
            </button>
          </form>
        ...
        {memories.length > 0 ? (
          memories.map((m) => (
            <div key={m.id} className="group relative animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="flex flex-col gap-1">
                <h3 className={`${getFontSize(m.importance)} tracking-tight leading-tight group-hover:text-sky-300 transition-colors`}>
                  {m.title}
                </h3>
                {m.detail && (
              ...
              <button
                ...
                className="text-zinc-800 hover:text-red-500 transition-all opacity-0 group-hover:opacity-100 absolute -right-2 top-0 p-1"
              >
                <LuTrash2 size={14} />
              </button>
            </div>
          ...
          <div className="flex flex-col items-center justify-center h-full opacity-10 py-10 scale-90">
            <LuBrain size={48} className="animate-pulse" />
            <p className="text-[11px] uppercase font-black tracking-[0.4em] mt-4">Memory Buffer Empty</p>
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <Panel className="bg-black/20 border-zinc-900/50 p-4">
      <div className="flex items-center gap-2 mb-4">
        ...
        <SectionLabel className="mb-0 text-[11px] text-sky-500 font-bold">Weekly Habit Pulse</SectionLabel>
      </div>
        ...
        {habits.map(habit => (
          <div key={habit.id} className="flex flex-col gap-2">
            <div className="flex justify-between items-center px-1">
              ...
              <DataText className="text-[9px] text-sky-500 font-black tracking-tighter">{habit.streak}D STREAK</DataText>
            </div>
